...
import { downloadBlob, estimateSizeMB } from "@/lib/export";
import { exportProjectOnServer } from "@/lib/server-export";
...
function EditorPage() {
  ...
  const doExport = async () => {
    ...
    try {
      const blob = await exportProjectOnServer(
        project,
      ...
      downloadBlob(blob, `${project.name.replace(/[^\w\u0600-\u06FF -]/g, "") || "player"}.mp4`);
      setLastExport({ mb: blob.size / (1024 * 1024), ext: "mp4" });
      toast.success(`تم التصدير · الحجم الفعلي ${(blob.size / (1024 * 1024)).toFixed(1)} ميجابايت`);
    } catch (e) {
      if (!cancelRef.current) toast.error(e instanceof Error ? e.message : "فشل التصدير");
    } finally {
        ...
        {exporting && (
          <div className="mb-4 rounded-2xl border border-border bg-panel p-4">
            <div className="mb-2 flex items-center justify-between text-sm">
              <span>
                جاري التصدير على السيرفر · {project.quality}p · {project.fps} إطار/ث · MP4
              </span>
            ...
            <Button
              ...
              onClick={() => {
                ...
                setExporting(false);
                toast.info("تم إيقاف متابعة التصدير");
              }}
            >
              إلغاء
            </Button>
            ...
            <Tabs defaultValue="audio">
              ...
              <TabsContent value="export" className="space-y-3">
                <Section title="إعدادات التصدير">
                  ...
                  <p className="text-xs text-muted-foreground">
                    الناتج: MP4 · صوت AAC · المدة {fmtTime(duration)} بالضبط. التصدير يتم على السيرفر، وتقدر تتابع تقدمه من هنا.
                  </p>
